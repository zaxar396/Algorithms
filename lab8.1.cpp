...
DWORD WINAPI thread(LPVOID param)
{
	...
	{
		GetClientRect(pdata->hWindow, &rect);
		hdc = GetDC(pdata->hWindow);

		pdata->pS->paint(hdc);
		Sleep(30);
		pdata->pS->clear(pdata->hWindow, hdc, rect);
		pdata->pS->move(rect);

		ReleaseDC(pdata->hWindow, hdc);
	}
...
LRESULT CALLBACK WindowProcess(HWND hWindow, UINT uMessage, WPARAM wParameter, LPARAM lParameter) {
	...
	{
	case WM_CREATE:
		Event = CreateEvent(0, true, false, L"Event");

		for (int i = 0; i < 9; i++)
		{
			data[i].pS = shapes[i];
			data[i].hEv = thr[i] = CreateEvent(0, false, false, 0);
			data[i].hWindow = hWindow;
			CreateThread(0, 0, thread, &data[i], 0, 0);
		}
		break;

	case WM_DESTROY:
		WaitForMultipleObjects(9, thr, TRUE, INFINITE);
